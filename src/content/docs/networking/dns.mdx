...
- **Tính sẵn sàng**: DNS đảm bảo dịch vụ luôn доступен, ngay cả khi máy chủ thay đổi vị trí hoặc cấu hình.
...
Để làm quen với khái niệm DNS, bạn có thể thực hiện các bước sau:
1. **Kiểm tra IP của một tên miền**:
    - Mở terminal (Linux/Mac) hoặc Command Prompt (Windows).
    - Gõ: `ping google.com`
    - Kết quả hiển thị IP, ví dụ: `142.250.190.78`.
    - Hiểu rằng DNS đã chuyển `google.com` thành IP này.
2. **So sánh truy cập bằng tên miền và IP**:
    - Truy cập `www.google.com` trong trình duyệt (dùng DNS).
    - Truy cập trực tiếp `142.250.190.78` (không dùng DNS).
    - Quan sát: Tên miền dễ nhớ hơn, nhưng cả hai đều dẫn đến cùng website.
3. **Kiểm tra DNS Resolver hiện tại**:
    - Linux/Mac: `cat /etc/resolv.conf` (hiển thị Resolver như `8.8.8.8`).
    - Windows: `ipconfig /all | findstr DNS`.
    - Ghi lại Resolver (ví dụ: Google DNS, ISP DNS).
4. **Thử đổi Resolver**:
    - Thay đổi DNS trên máy tính sang Google DNS (`8.8.8.8`, `8.8.4.4`).
    - Kiểm tra tốc độ truy cập bằng `ping facebook.com` trước và sau khi đổi.

### 5. Vai trò của DevOps trong DNS
...
- Người dùng nhập tên miền (`example.com`) vào trình duyệt.
    - **Resolver** nhận truy vấn và hỏi **Root Server**.
    - **Root Server** trả về địa chỉ của **TLD Server** (như `.com`).
    - **TLD Server** trả về địa chỉ của **Name Server** của tên miền (như `ns1.example.com`).
    - **Name Server** trả về **DNS Record** từ **Zone** (như bản ghi A: `192.0.2.1`).
    - **Registrar** là nơi bạn cấu hình Name Server cho tên miền.

### 9. Thực hành cơ bản

1. **Kiểm tra Name Server của một tên miền**:
    - Mở terminal (Linux/Mac) hoặc Command Prompt (Windows).
    - Gõ: `dig example.com NS`
    - Kết quả hiển thị Name Server, ví dụ: `ns1.example.com`, `ns2.example.com`.
2. **Kiểm tra Resolver đang sử dụng**:
    - Trên Linux/Mac: `cat /etc/resolv.conf` để xem Resolver (như `8.8.8.8`).
    - Trên Windows: `ipconfig /all | findstr DNS` để xem DNS Server.
3. **Tìm thông tin Registrar**:
    - Dùng lệnh: `whois example.com` hoặc truy cập https://whois.domaintools.com.
    - Kết quả hiển thị Registrar (như Namecheap) và Name Server.
4. **Tạo subdomain giả lập**:
    - Nếu có quyền truy cập vào Registrar (như Namecheap), thử thêm subdomain `test.example.com` với bản ghi A trỏ đến IP `192.0.2.1`.
    - Kiểm tra bằng: `ping test.example.com`.

### 10. Vai trò của DevOps với các thành phần DNS
...
#### 2.7. SOA Record (Start of Authority Record)
